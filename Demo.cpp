...
int main() {
range a(0,4),b(1,5),c(2,4),d(3,3);
chain e(a,b); 
chain f(c,d);
chain g(e,f);
chain<chain<range<int>,range<int>>,chain<range<int>,range<int>>>::iterator it = g.begin();
for(;it!=g.end();++it){
	cout <<*it <<endl;

}

// 	 for (int i : chain(chain(range(0,4), range(1,5)), chain(range(2,4), range(3,3))))
// 		{
// 			//cout << i << endl;
// 		}
